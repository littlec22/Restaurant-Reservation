<?php
    ...
    }elseif(isset($_GET['paymentForm'])){
       paymentForm($_GET['paymentForm']);
        ...
        global $con;
            ...
            $res1 = mysqli_fetch_array($sql);
                if($res1){
                    ...
                    $dateSubmit = date_create(date("Y-m-d H:i:s a"));
                    $query1 = "INSERT INTO `event_information`(`event_system_id`, `typeEvent`, `areaEvent`, `paxEvent`, `repEvent`, `dateEvent`, `startEvent`, `endEvent`, `packageCost`, `perheadCost`, `addservicesCost`, `addfoodCost`, `totalCost` , `event_hours`, `area_rate`,`cust_id`,`event_status`,`payment_status`,`date_submit`) VALUES ('$event_system_id','$typeEvent','$areaEvent','$paxEvent','$repEvent','$dateEvent' ,'$startEvent','$endEvent','$packagePrice','$packagePerHead','$addServices','$addFood','$total','$eventHours','$areaRate','".$_SESSION['cust_id']."','Pending','Unpaid',$dateSubmit'')";
...
function loadEventInfo($id){
                    ...
                    ?>
                        <h1 id="remainingDays" ></h1>
                            <tr>
                                <td colspan="2">
                                    <?php
                                    if($event_status =="Reserve"){
                                        ?>
                                        <div class="col-lg-4">
                                            <a class="btn btn-block btn-primary " style="margin-bottom:10px;"  onclick="try{paymentEvent('<?php echo $event_id; ?>');}catch(err){alert(err.message);}" > Add Payment</a>
                                    ...
                                    ?>
                                    <div class="col-lg-4">
                                        <a class="btn btn-block btn-success " style="margin-bottom:10px;" onclick="try{updateEvent('<?php echo $event_id; ?>');}catch(err){alert(err.message);}">Update Event</a>
                                    </div>
                                    <div class="col-lg-4">
                    ...
                    <div class="modal-dialog">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-body" style="padding:40px 50px;">
                                ...
                                <a class="btn btn-block btn-danger" style="margin-bottom:10px;  display:none;" id="cancel" onclick="cancelEvent('<?php echo $event_id; ?>');">Cancel Event</a>
    ...
    <?php
                if($query5 =  mysqli_query($con,"SELECT `payment2_id`, `payment`, `proof_of_payment` , `status` FROM `payment2_table` WHERE `payment_id` = '".$res4['payment_id']."' ")or die(mysqli_error($con)."4")){
                    $xP=1;
                    $totalP=0;
                    while($res5 = mysqli_fetch_assoc($query5)){
                        ...
                        <tr <?php if(($status=='pending')||($status=='disapproved')){ echo "class='danger' ";} ?>>
                            <td>Payment  <?php echo $xP; echo "  -  ".$status; ?></td>
                            <td><?php echo $p;?> - <a href="#proofOfBilling<?php echo $xP; ?>" class="" data-toggle="collapse">Proof of Billing</a></td>
                            <div id="proofOfBilling<?php echo $xP;?>" class="collapse">
                                <img id="proofOfBillingImg" src="data:image/png;base64,<?php echo base64_encode($res5['proof_of_payment']);?>" style="vertical-align: middle;">
                              </div>
                        </tr>
                        <?php
                        $xP++;
    ...
    ?>
                        ...
                        <tr>
                            ...
                            <td><h2><?php echo ($res4['contract_payment'] -  $totalP );  ?></h2></td>
                        </tr>        
...
function paymentForm($id){
    global $con;
                    ...
                    $totalpayment = 0 ;
    $totalcost;
    if($query =  mysqli_query($con,"SELECT  `event_system_id`, `totalCost` FROM `event_information` WHERE `event_id` = '$id' ")or die(mysqli_error($con)."1")){
                    
                    while($res = mysqli_fetch_assoc($query)){
                        $totalcost = $res['totalCost'];
                        if($query1 =  mysqli_query($con,"SELECT `payment_id`, `contract_payment`  FROM `payment_table` WHERE `event_id` = '".$res['event_system_id']."'")or die(mysqli_error($con)."1")){
                            
                            while($res1 = mysqli_fetch_assoc($query1)){
                                $countP1++;
                                $payment_id = $res1['payment_id'];
                                $cpayment = $res1['contract_payment'];
                            }
                        }
                    }
    }
    if($query =  mysqli_query($con,"SELECT  `payment` FROM `payment2_table` WHERE `payment_id`='$payment_id' AND `status` = 'approved' ")or die(mysqli_error($con)."1")){
                    
                    while($res = mysqli_fetch_assoc($query)){
                        $totalpayment = $totalpayment + $res['payment'];
                    }
    }
    if($countP1 != 0){
        ?>
                               <form action="f/function.php" method="post" enctype="multipart/form-data">
                                    <div id="payment-contract-amount-div" >
                                          <label class="control-label" for="usrname">Contract Amount</label>
                                        <h2 type="text" class="form-control" id="payment-contract-amount"   ><?php echo $cpayment;?></h2>

                                    </div>
                                    <div id="payment-contract-amount-div" >
                                          <label class="control-label" for="usrname">Your Total Active Payment</label>
                                        <h2 type="text" class="form-control" id="total-payment-amount" ><?php echo $totalpayment;?></h2>

                                    </div>

                                        <div id="payment-amount-div" class="form-group">
                                          <label class="control-label" >Payment</label>
                                          <input type="text" class="form-control" id="payment-amount" name="payment-amount" value="0" onchange="document.getElementById('payment-balance').innerHTML = Number(document.getElementById('payment-contract-amount').innerHTML) - (Number(this.value) + Number(document.getElementById('total-payment-amount').innerHTML));">
                                             <span id="paymentx1" class="glyphicon glyphicon-remove form-control-feedback" style="visibility:hidden;"></span>
                                        </div>
                                       <div id="payment-proof-div" class="form-group">
                                          <label class="control-label" >Proof of payment (any image of your bill of your payment)</label>
                                          <input type="file" class="form-control" name="proof-payment" id="proof-payment-amount"  >
                                             <span id="paymentx1" class="glyphicon glyphicon-remove form-control-feedback" style="visibility:hidden;"></span>
                                        </div>
                                        <div  class="form-group">
                                            <h2>Balance : </h2>
                                            <h2 id="payment-balance"><?php echo ($cpayment - $totalpayment); ?></h2>
                                        </div>
                                        <input type="hidden" value="<?php echo $payment_id;?>" name="payment_id" >
                                        <input type="hidden" value="<?php echo $id;?>" name="event_id" >
                                        </p>

                                    <input style="margin-bottom:10px; display:none" id="payment-submit"  type="submit" name="paymentSave" value="Submit">
                                    <a  class="btn btn-block btn-success"style="margin-bottom:10px; " onclick="if(confirm('Are you sure to submit you payment? ')==true){document.getElementById('payment-submit').click();}" > Submit</a>
                                </form>
                                <?php
    }else{
        ?>
                                <form action="f/function.php" method="post" enctype="multipart/form-data">
                                    <div id="payment-contract-amount-div" >
                                          <label class="control-label" for="usrname">Contract Amount</label>
                                        <h2 type="text" class="form-control" id="payment-contract-amount"   ><?php echo $totalcost;?></h2>

                                    </div>
                                    <div id="payment-contract-amount-div" >
                                          <label class="control-label" for="usrname">Your Total Active Payment</label>
                                        <h2 type="text" class="form-control" id="total-payment-amount"  >0</h2>

                                    </div>

                                        <div id="payment-amount-div" class="form-group">
                                          <label class="control-label" >Payment</label>
                                          <input type="text" class="form-control" id="payment-amount" name="payment-amount" value="0" onchange="document.getElementById('payment-balance').innerHTML = Number(document.getElementById('payment-contract-amount').innerHTML) - (this.value + Number(document.getElementById('total-payment-amount').innerHTML));">
                                             <span id="paymentx1" class="glyphicon glyphicon-remove form-control-feedback" style="visibility:hidden;"></span>
                                        </div>
                                       <div id="payment-proof-div" class="form-group">
                                          <label class="control-label" >Proof of payment (any image of your bill of your payment)</label>
                                    ...
                                    <input id="payment-submit"  type="submit" name="paymentSubmit" value="Submit"  style="margin-bottom:10px; display:none">
                                    <a  class="btn btn-block btn-success"style="margin-bottom:10px; " onclick="if(confirm('Are you sure to submit you payment? ')==true){document.getElementById('payment-submit').click();}" > Submit</a>
...
function paymentSubmit(){
    global $con;
     $datestatic = date_create(date("Y-m-d h:i:s a"));
    
    $payment_id = $_POST['payment_id'];
    $id = $_POST['event_id'];
    $payment = $_POST['payment-amount'];
			$payment_img_name = addslashes($_FILES['proof-payment']['name']);
            $imageData = mysqli_real_escape_string($con,file_get_contents($_FILES['proof-payment']['tmp_name']));
    
			$query = mysqli_query($con,("INSERT INTO `payment2_table`( `payment_id`, `payment`, `proof_of_payment` , `status`,`date_submit`) VALUES ('$payment_id','$payment','$imageData','pending','".date_format($datestatic,"Y-m-d h:i:s a")."')")) or die(mysqli_error());
			
			if($query){
                header("location: ../calendar.php?loadEvent=$id");
            }
}
?>
